...
class ForestDesigner extends EditorWindow {
	...
	private var useWatermarkedArt		: boolean = true;
	...
	private var theCurrSeason			: int = 1;
	private var theEffect				: int = 0;
	private var numOfTreeRoots 			: int = 5+1;
	private var numOfTreeRootsModular	: int = 4+1;
	private var numOfTreeTop			: int = 10+1;
	private var numOfTreeTrunk			: int = 1+1;
	private var numOfTreeLeavesAll		: int = 3+1;
	private var numOfTreeLeavesBulk		: int = 3+1;
	private var numOfPlants				: int = 16+1;
	private var numOfRocks 				: int = 15+1;
		...
		switch(wizardPage){
			case 0:
				if (runOnceBool){
					...
					if ( PlayerPrefs.GetInt("daSeason") == 0){
						theCurrSeason = 0;
					} else if ( PlayerPrefs.GetInt("daSeason") == 1){
						theCurrSeason = 1;
					} else if ( PlayerPrefs.GetInt("daSeason") == 2){
						theCurrSeason = 2;
					} else {
						theCurrSeason = 0;
						PlayerPrefs.SetInt("daSeason", 0);
					}
			...
			case 1:
				var options : String[] = ["Random", "Summer", "Autumn"];
				var options2 : String[] = ["None", "Blur"];
				...
				GUILayout.BeginArea (Rect (10,25,250,500));
				useWatermarkedArt = EditorGUI.Toggle(Rect(0,5,position.width,20),
				...
				if(GUILayout.Button(AssetDatabase.LoadAssetAtPath(HPFDpath + "/Editor/WizardImages/Btn_GenTree.psd", Texture2D) as Texture2D
					...
					if ( useWatermarkedArt == false || Random.value > 0.5 ){
						GenerateObjectName( "treeRoots", true, true, "N", "H", 1, "01", "BasicRoots" );
						CreateElement ( theCreationFileName, mainPath, 0, 0, 0, 1, 1, 1 );
					} else {
						switch (theEffect){
							case 0:
								mainPath = ( HPFDpath + "/Modular/Trees_N" );
								CreateElement ( "HPFD_TRT01N_BASE01HN_0", mainPath, 0, 0, -1, 1, 1, 1 );
							break;
							case 1:
								mainPath = ( HPFDpath + "/Modular/Trees_B" );
								CreateElement ( "HPFD_TRT01N_BASE01HB_0", mainPath, 0, 0, -1, 1, 1, 1 );
							break;
						}
						for ( i = 1; i <= 4; i++) {
							tempString = "0" + i.ToString();
							GenerateObjectName( "treeRoots", true, true, "N", "H", 1, tempString, "01" );
							CreateElement ( theCreationFileName, mainPath, 0, 0, -2, 1, 1, 1 );
							if (Random.value > 0.8){
								i = i - 1;
							}
						}
					...
					if (Random.value > 0.8){
						...
						for ( i = 1; i < 10; i++) {
							...
							if (useWatermarkedArt){
								switch ( theCurrSeason ){
									case 1:
										GenerateObjectName( "treeLeavesBulk", true, true, "S", "H", 1, tempString, tempStringTwo );
									break;

									case 2:
										GenerateObjectName( "treeLeavesBulk", true, true, "A", "H", 1, tempString, tempStringTwo );
									break;
								}
							} else {
								GenerateObjectName( "treeLeavesBulk", true, true, "S", "H", 1, tempString, tempStringTwo );
				...
				if (daMainObject != null){
					...
					if ( daMainObject.transform.childCount > 0 ){
						...
						if ( useWatermarkedArt ) {
							GUILayout.BeginArea (Rect (10,290,250,500));
							theEffect = EditorGUI.Popup(
							Rect(0,0,200, 20),
							"Effect:",
							theEffect, 
							options2);
				...
				if(GUILayout.Button(buttonBack, GUILayout.Width(145), GUILayout.Height(45))) {
					runOnceBool = true;
					PlayerPrefs.SetInt("daSeason", theCurrSeason);
					wizardPage = 0;
				}
				GUILayout.EndArea ();
			...
			case 2:
					...
					for (i = 0; i < daInt1; i++) {
						if (useWatermarkedArt){
							switch ( theCurrSeason ){
								...
								case 2:
									GenerateObjectName( "plants", true, true, "A", "H", 1, tempString, "01" );
								break;
							}
						} else {
							GenerateObjectName( "plants", true, true, "S", "H", 1, tempString, "01" );
				...
				if (daMainObject){
					...
					if (daMainObject.transform.childCount > 0){
							...
							for (i = 0; i < daInt1; i++) {
								if (useWatermarkedArt){
									switch ( theCurrSeason ){
										...
										case 2:
											GenerateObjectName( "plants", true, true, "A", "H", 1, "01", "01" );
										break;
									}
								} else {
									GenerateObjectName( "plants", true, true, "S", "H", 1, "01", "01" );
						...
						if ( useWatermarkedArt ) {
							GUILayout.BeginArea (Rect (10,200,250,500));
							theEffect = EditorGUI.Popup(
							Rect(0,0,200, 20),
							"Effect:",
							theEffect, 
			...
			case 4:
				...
				GUILayout.BeginArea (Rect (10,80,250,500));
				defaultQualityInt = EditorGUI.Popup(
				Rect(0,0,200, 20),
		...
		switch (daObjectName){
			case "treeRoots":
				...
				if (stringTwo == "BasicRoots"){
					...
					if (daRandomizer){
						designNumber = Random.Range(1, numOfTreeRootsModular);
					} else {
						if (nextDesign){
							currDesignNum = currDesignNum++;
							if ( currDesignNum >= numOfTreeRoots ){
					...
					theCreationFileName = "HPFD_TRT01"+theSeason+"_MD"+stringOne+designNumberString+daQuality+daEffects+"_0";
				}
			...
			case "treeTrunk":
				mainPath = ( HPFDpath + "/Modular/Trees_" + daEffects );
				if (daRandomizer){
					designNumber = Random.Range(1, numOfTreeTrunk);
				} else {
					if (nextDesign){
						currDesignNum = currDesignNum++;
						if ( currDesignNum >= numOfTreeTrunk ){
							currDesignNum = 1;
						}
					} else {
						currDesignNum = currDesignNum - 1;
						if ( currDesignNum == 0 ){
							currDesignNum = numOfTreeTrunk - 1;
						}
					}
				}
				if (designNumber<10){
				...
				theCreationFileName = "HPFD_TTR"+designNumberString+theSeason+"_BR0101"+daQuality+daEffects+"_0";
			break;
			case "treeTop":
				mainPath = ( HPFDpath + "/Modular/Trees_" + daEffects );
				if (daRandomizer){
			...
			case "treeLeavesAll":
				mainPath = ( HPFDpath + "/Modular/Trees_" + daEffects );
				if (daRandomizer){
					designNumber = Random.Range(1, numOfTreeLeavesAll);
				} else {
					if (nextDesign){
						currDesignNum = currDesignNum++;
						if ( currDesignNum >= numOfTreeLeavesAll ){
							currDesignNum = 1;
						}
					} else {
						currDesignNum = currDesignNum - 1;
						if ( currDesignNum == 0 ){
							currDesignNum = numOfTreeLeavesAll - 1;
						}
					}
				}
				if (designNumber<10){
				...
				theCreationFileName = "HPFD_TTP"+stringOne+theSeason+"_EV"+designNumberString+"01"+daQuality+daEffects+"_0";
			break;
			case "treeLeavesBulk":
				mainPath = ( HPFDpath + "/Modular/Trees_" + daEffects );
				if (daRandomizer){
	...
	function SaveTextureToFile(fileName : String, daImageSizeX : float, daImageSizeY : float)
	{
		...
		settings.spriteAlignment 				= SpriteAlignment.Custom;
		...
		AssetDatabase.ImportAsset(HPFDpath + "/SavedImages/" + fileName + "_HQ" + ".png", ImportAssetOptions.ForceUpdate);
	...
	function LowerQualityTexture(textureName : String, saveFileName : String, qualityFloat : float)
	{
		...
		if (primaryTexture){
			...
			AssetDatabase.ImportAsset(HPFDpath + "/SavedImages/" + saveFileName + ".png", ImportAssetOptions.ForceUpdate);	
		}
	}
	...
	function EffectGreyscale(textureName : String, saveFileName : String){
		...
		if ( !assetCheckBool ){
			...
			if (primaryTexture){
				...
				importer.textureType 			= TextureImporterType.Sprite;
				importer.maxTextureSize 		= 2048;
				importer.spritePixelsPerUnit 	= primaryTexture.pixelsPerUnit;
				importer.textureFormat 			= TextureImporterFormat.AutomaticTruecolor;
				importer.filterMode 			= FilterMode.Bilinear;
				importer.spritePivot 			= Vector2(0,0);
		...
		if (daMainObject.GetComponent.<SpriteRenderer>() && daMainObject.GetComponent.<SpriteRenderer>().sprite != null && daMainObject.GetComponent.<SpriteRenderer>().sprite.name.Substring(0,5) == "HPFD_"){
			...
			if ( assetCheckBool2 ){
				if(GUILayout.Button(AssetDatabase.LoadAssetAtPath(HPFDpath + "/Editor/WizardImages/Btn_MakeEditable.psd", Texture2D) as Texture2D
					...
					tempX2 = daMainObject.transform.GetChild(0).GetComponent.<SpriteRenderer>().sprite.texture.width;
					tempY2 = daMainObject.transform.GetChild(0).GetComponent.<SpriteRenderer>().sprite.texture.height;
					for ( var i = 0; i < (daMainObject.transform.childCount); i++) {
						for ( var y = 0; y < daMainObject.transform.GetChild(i).GetComponent.<SpriteRenderer>().sprite.texture.height; y++ ){
							for ( var x = 0; x < daMainObject.transform.GetChild(i).GetComponent.<SpriteRenderer>().sprite.texture.width; x++ ){
								if (daMainObject.transform.GetChild(i).GetComponent.<SpriteRenderer>().sprite.texture.GetPixel(x,y).a > 0){
									if ( daMainObject.transform.GetChild(i).localPosition.x <= daMainObject.transform.GetChild(0).localPosition.x ){
										if (x < (tempX2 )){
											tempX2 = x;
										}
									}
									if ( daMainObject.transform.GetChild(i).localPosition.y <= daMainObject.transform.GetChild(0).localPosition.y ){
										if (y < (tempY2 )){
											tempY2 = y;
										}
									}
					...
					daMainObject.transform.localPosition.x = PosX - tempX2;
					daMainObject.transform.localPosition.y = PosY - tempY2;
					daMainObject.transform.localPosition.z = PosZ;
					RefreshPosition(daMainObject);
				}
			} else {
